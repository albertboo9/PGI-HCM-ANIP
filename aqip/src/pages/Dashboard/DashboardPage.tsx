...
export default function DashboardPage() {
  ...
  ] as any; // Cast as any to fix type checking easily for the mock

  return (
    <div className="space-y-6 h-full flex flex-col">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Executive Cockpit — Vision Nationale</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 shrink-0">
        <AQIPCard className="flex items-center gap-4 border-aqip-accent/30 bg-gradient-to-br from-aqip-bg-surface to-aqip-accent/5">
          ...
          <div>
            ...
            <div className="text-2xl font-bold text-white">76 <span className="text-sm text-aqip-text-muted">/ 100</span></div>
          </div>
        </AQIPCard>
        ...
        <AQIPCard className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-2xl font-bold text-white">18,524</div>
          </div>
        </AQIPCard>
          ...
          <div className="p-4 border-b border-aqip-border flex justify-between items-center bg-aqip-bg-surface shrink-0 z-10">
            ...
            <div className="flex bg-aqip-bg-elevated rounded-md p-1">
              <button 
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-sm transition-colors ${activeLayer === 'qualite' ? 'bg-aqip-primary text-white shadow' : 'text-aqip-text-muted hover:text-white'}`}
              >
                Qualité (IQSP)
              </button>
              <button 
                onClick={() => setActiveLayer('satisfaction')}
                className={`px-3 py-1.5 text-xs font-medium rounded-sm transition-colors ${activeLayer === 'satisfaction' ? 'bg-aqip-primary text-white shadow' : 'text-aqip-text-muted hover:text-white'}`}
              >
          ...
          <div className="space-y-4 flex-1">
            <div className="bg-aqip-danger/10 border border-aqip-danger/20 p-4 rounded-lg">
              <div className="flex justify-between items-start mb-2">
                <div className="font-semibold text-white">Natitingou Centre</div>
              ...
              <button className="w-full py-2 bg-aqip-bg-surface border border-aqip-danger/30 text-xs text-white font-medium rounded hover:bg-aqip-danger/20 transition-colors">
            ...
            <div className="bg-aqip-warning/10 border border-aqip-warning/20 p-4 rounded-lg">
              <div className="flex justify-between items-start mb-2">
                <div className="font-semibold text-white">Parakou Centre</div>
              ...
              <button className="w-full py-2 bg-aqip-bg-surface border border-aqip-warning/30 text-xs text-white font-medium rounded hover:bg-aqip-warning/20 transition-colors">
